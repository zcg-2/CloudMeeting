...
void MyTcpSocket::sendData(MESG* send)
{
	...
	//消息类型
    qToBigEndian<quint16>(send->msg_type, sendbuf + bytestowrite);//写入到buf
	bytestowrite += 2;
    ...
    else
    {
        ...
        if ((quint64)data_size + 1 + MSG_HEADER <= hasrecvive) //收够一个包
        {
            if (recvbuf[0] == '$' && recvbuf[MSG_HEADER + data_size] == '#') //且包结构正确
            {
                ...
                qDebug() << "recv data type: " << msgtype;//获取消息类型用于判断
...
bool MyTcpSocket::connectToServer(QString ip, QString port, QIODevice::OpenModeFlag flag)
{
	...
	bool retVal;
    QMetaObject::invokeMethod(this, "connectServer", Qt::BlockingQueuedConnection, Q_RETURN_ARG(bool, retVal),//阻塞调用"connectServer"
